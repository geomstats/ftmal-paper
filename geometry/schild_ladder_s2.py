...
def main():
    ...
    base_point = gs.array([-1., -1., -0.5]) / gs.sqrt(2.25)
    tangent_vec_b = SPACE.random_uniform()
    ...
    ladder = METRIC.ladder_parallel_transport(
        tangent_vec_a, tangent_vec_b, base_point, scheme='schild',
        n_rungs=N_STEPS, return_geodesics=True)

    schild_ladder = ladder['transported_tangent_vec']
    ...
    for points in trajectory:
        ...
        sphere_visu.draw_points(ax, final_geodesic(t), marker='o', c='green', s=2)
        sphere_visu.draw_points(ax, diagonal(t), marker='o', c='r', s=2)

    tangent_vectors = gs.stack(
        [tangent_vec_b, tangent_vec_a, schild_ladder], axis=0) / N_STEPS
